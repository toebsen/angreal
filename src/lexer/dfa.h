 ...
 * @tparam State -> State Enum type [note: needs valid State(-1) for invalid
 * state]
 */

template <typename Input, typename State>
class DFA {
    using transition_container_t =
        std::multimap<std::pair<State, Input>, State>;
    using state_container = std::set<State>;
   ...
   public:
    ...
    void addTransition(const State src, const Input input, const State dest) {
        ...
        if (!states_.contains(dest)) {
            throw UnknownStateException(dest);
        }
    ...
    void reset() { current_state_ = initial_state_; }

    State input(const Input input) {
        if (auto it = transitions_.find({current_state_, input});
            it != transitions_.end()) {
    ...
    bool isAccepted() { return final_states_.contains(current_state_); }
    ...
    State state() { return current_state_; }
