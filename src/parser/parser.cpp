...
std::shared_ptr<AST::Program> Parser::parseProgram(const std::vector<Token> &tokens)
{
  auto statements = AST::nodes_t();
  current_token = tokens.begin();
  next_token = tokens.begin() + 1;

  while (current_token->type() != Token::Type::EndOfProgram && current_token != tokens.end()) {
...
std::shared_ptr<AST::Expression> Parser::parseExpression(const std::vector<Token> &tokens)
{
  current_token = tokens.begin();
  next_token = tokens.begin() + 1;
...
void Parser::expectToken(Token::Type t)
{
  if (current_token->type() != t) {
    std::stringstream ss;
    ss << "Expected " << Token::type2str(t) << ", but got: " << Token::type2str(current_token->type());
...
void Parser::expectTokensOneOf(std::initializer_list<Token::Type> types)
{
  ...
  expectToken(description, [&](Token::Type t) {
    return allowed.contains(t);
  });
}

void Parser::expectToken(const std::string &description, std::function<bool(Token::Type t)> predicate)
{
  if (!predicate(current_token->type())) {
    std::stringstream ss;
    ss << "Expected " << description << ", but got: " << Token::type2str(current_token->type());
    ss << "('" << current_token->value() << "')"
       << " in line: " << std::to_string(current_line_number);
    error(ss.str());
  }
...
std::shared_ptr<AST::Expression> Parser::parseSimpleExpression()
{
  ...
  if (next_token->type() == Token::Type::AdditiveOp || next_token->type() == Token::Type::AndStatement) {
    consume();
    auto opType = current_token->value();
    consume();
    auto rhs = parseTerm();
...
std::shared_ptr<AST::Expression> Parser::parseTerm()
{
  ...
  if (next_token->type() == Token::Type::MulOp || next_token->type() == Token::Type::DivOp || next_token->type() == Token::Type::OrStatement) {
    consume();
    auto opType = current_token->value();
    consume();
...
{
  if (current_token->type() == Token::Type::Boolean || current_token->type() == Token::Type::Integer || current_token->type() == Token::Type::Float || current_token->type() == Token::Type::String) {
    TypeSystem::Type decl = TypeSystem::mapTokenToLiteralType(current_token->type());
  ...
  } else if (current_token->type() == Token::Type::AdditiveOp || current_token->type() == Token::Type::Exclamation) {
    auto opType = current_token->value();
    consume();
...
{
  consume();
  expectTokensOneOf({ Token::Type::IntIdentifier, Token::Type::BoolIdentifier, Token::Type::FloatIdentifier, Token::Type::StringIdentifier });
...
std::shared_ptr<AST::FormalParameter> Parser::parseFormalParameter()
{
  ...
  TypeSystem::Type param_type = TypeSystem::mapTokenToDeclarationType(current_token->type());
  consume();
...
std::shared_ptr<AST::FunctionDeclaration> Parser::parseFunctionDeclaration()
{
  ...
  return std::make_shared<AST::FunctionDeclaration>(return_type, identifier, parameters, block->statements);
}
