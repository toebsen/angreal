...
TEST(DFA_Test, UnknownStateException) {
  ...
  EXPECT_THROW(dfa.add_transition(State::S0, {'0'}, State::S1), DFA_t::UnknownStateException);
  EXPECT_THROW(dfa.add_transition(State::S2, {'0'}, State::S0), DFA_t::UnknownStateException);
}

TEST(DFA_Test, StateAlreadyKnownException) {
  DFA_t dfa(State::S0, true);

  EXPECT_THROW(dfa.addState(State::S0, true), DFA_t::StateAlreadyKnownException);
